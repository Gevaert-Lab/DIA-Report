...
-   `cofounder_list`: List of the confounder names to use in the analysis
...
-   `Sample`: Sample name. This will be used in all the plots, so it must be meaningful and not too long.
-   `Run`: This is the raw file name *without the file format mzML/.d/.raw* processed
...
Each report generated from both protein and peptide analysis contains the following sections:
...
-   if more than two comparisons are present, upset plots are generated.
