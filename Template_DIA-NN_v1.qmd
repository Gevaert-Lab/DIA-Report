...
if (check_groups$status == 1){
  ...
  check_cofounder_list<-checkCofounderList(cofounderList= params$cofounder_list, colsDesign=colnames(design))
...
if (all(params$cofounder_list != '')) {
  ...
  ggplot(cData_n, aes(x=Numeric_Variables, y=Value, fill=Group))+geom_boxplot()
...
for (v in var_topca) {
  ...
  pca_ <- ggplot(data=prcompPe$x) + ggtitle(paste0("PCA by ",v)) +geom_point(aes(x=PC1, y=PC2, colour= colData(pe)[[v]]), size=3) + xlab(paste("PC1", percent(summary(prcompPe)$importance[,"PC1"][[2]], accuracy = 0.1)))  + ylab(paste("PC2", percent(summary(prcompPe)$importance[,"PC2"][[2]], accuracy = 0.1)))
