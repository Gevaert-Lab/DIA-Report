...
#pe <- filterFeatures(pe , ~contains(Protein.Ids, "Cont_"))
...
for (v in var_topca) {
  ...
  pca_ <-pe[["proteinRS"]] %>%
        ...
        t() %>%
        prcomp() %>%
        fviz_pca_ind(habillage = colData(pe)[[v]]  , geom=c("point"), addEllipses=FALSE, repel = FALSE ,title = paste0("PCA by ",v))
  plot(pca_)
...
upset(fromList(listInput), order.by = "freq",text.scale= 1)
dev.off()
