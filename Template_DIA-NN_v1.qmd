...
DEP_volcano <- function ( label, data ,  imagesDir ,p= params){
 ...
 render_child <- function(data, template) {
...
#| results: asis
  for (cmp in names(test)) {
    ...
    render_child(data = test[[cmp]],template='_templatePval_distribution.Rmd') 
    cat('\n\n')
...
#| results: asis
  ...
  for (cmp in names(test)) {
    ...
    render_child(data = test[[cmp]],template='_templateContrast.Rmd') 
    cat('\n\n')
