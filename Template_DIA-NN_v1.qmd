...
required_packages = c("QFeatures","msqrob2","readxl","SummarizedExperiment","visdat","ggrepel", "dplyr","tidyverse","magrittr","here","tidyr","plotly","DT","tibble","MSnbase",
                    "factoextra","hrbrthemes","gridExtra","UpSetR",'logger', 'scales','pander','heatmaply')
...
-   `r if (params$filtering_contaminats) 'Contaminants proteins are filtered ' else 'Contaminants proteins are not filtered'`
 ...
 pe <- filterFeatures(pe, ~ nNonZero >= round(size  * ( params$nNonZero / 100))   )
  ...
  if (params$filtPerGroup) {
      ...
      for ( v in group_val){
          val = paste0('perc',v)
          log_info(val)
           #pe <- filterFeatures(pe, ~  as.f1- val <  params$nNonZero )
...
log_info('PEptide per proteins filt')
...
pe <- aggregateFeatures(pe, i = "precursorNorm",
 ...
 log_info('Protein level % not missing')
for ( v in group_val){
    val = paste0('Zero',v)
    log_info(val)
...
if (all(params$confounder_list != '')) {
  ...
  print(boxplot_num)
               pdf(file= file.path(params$folder_prj,"Result",paste0('Boxplot_numerical_conf.pdf')),paper="a4")
               boxplot_num <-ggplot(cData_n, aes(x=Numeric_Variables, y=Value, fill=Group))+geom_boxplot()
  print(boxplot_num)
      invisible(dev.off())
  ...
  for (col_ch in unlist(list_character)){
    ...
    print(boxplot_ch)
          pdf(file= file.path(params$folder_prj,"Result",paste0('Boxplot_character_conf.pdf')),paper="a4")
         boxplot_ch<- ggplot(cData_n, aes(x={{col_ch}}, fill=Group))+geom_bar()+ggtitle( {{col_ch}} )
    print(boxplot_ch)  
...
if (length(str_which(var_topca, "-"))== 0){
  ...
  pca_ <- ggplot(data=prcompPe$x) + ggtitle(paste0("PCA by ", var_topca)) +geom_point(aes(x=PC1, y=PC2, colour= colData(pe)[[var_topca]]), size=3) + xlab(paste("PC1", percent(summary(prcompPe)$importance[,"PC1"][[2]], accuracy = 0.1))) + ylab(paste("PC2", percent(summary(prcompPe)$importance[,"PC2"][[2]], accuracy = 0.1))) + labs(colour=var_topca)
   ...
   pdf(file= file.path(params$folder_prj,"Result",paste0("PCA_", var_topca, ".pdf")),paper="a4")
     plot(pca_)
...
#| results: asis
  for (cmp in names(test)) {
    ...
    render_child(data = test[[cmp]], 
                 pe =pe , 
                 label= cmp ,
                 path=file.path(params$folder_prj, "Result","Contrast",cmp), template='_templatePval_distribution.Rmd') 
    cat('\n\n')
