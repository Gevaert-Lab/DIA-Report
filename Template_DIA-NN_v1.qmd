...
if (check_groups$status == 1){
  ...
  check_cofounder_list<-checkCofounder(cofounder= params$cofounder_list, colsDesign=colnames(design))
if (check_cofounder_list$status == 1){
    stop(check_cofounder_list$error)
}
#log_info(params$cofounder_list[1]) 
...
if (length(custom_col) >= 1){
    for (col_add in custom_col){
      ...
      if  (is.character(design[[col_add]])) {
          design[[col_add]]<- as.factor(design[[col_add]])
  	   }
      
      }
}

  check_cofounder_PCA<-checkCofounder(cofounder= unique(unlist(str_split(params$PCA_comparison, "-"))), colsDesign=colnames(colData(pe)))
if (check_cofounder_PCA$status == 1){
    stop(check_cofounder_PCA$error)
}
...
## Cofounder Values Analysis
```{r cofounder_value_analysis}
# saveRDS(pe, file="C:\\Users\\catel\\OneDrive\\Documenti\\GitHub\\DIA-Report\\pe-obj.rds")
cData<-as.data.frame(colData(pe))
list_numeric <- list()
list_character <- list()

if (all(params$cofounder_list != '')) {
  for  (col in params$cofounder_list){
     if  (is.character(cData[[col]])) {
...
if (length(str_which(var_topca, "-"))== 0){
  ...
  pca_ <- ggplot(data=prcompPe$x) + ggtitle(paste0("PCA by ", var_topca)) +geom_point(aes(x=PC1, y=PC2, colour= colData(pe)[[var_topca]]), size=3) + xlab(paste("PC1", percent(summary(prcompPe)$importance[,"PC1"][[2]], accuracy = 0.1)))  + ylab(paste("PC2", percent(summary(prcompPe)$importance[,"PC2"][[2]], accuracy = 0.1)))
   ...
   plot(pca_)
	...
	for (v in var_topca) {
	  ...
	  break
		  ...
		  pca_ <- ggplot(data=prcompPe$x) + ggtitle(paste0("PCA by ", v)) +geom_point(aes(x=PC1, y=PC2, colour= colData(pe)[[num_comp]],shape = colData(pe)[[chr_comp]]), size=3 ) + xlab(paste("PC1", percent(summary(prcompPe)$importance[,"PC1"][[2]], accuracy = 0.1)))  + ylab(paste("PC2", percent(summary(prcompPe)$importance[,"PC2"][[2]], accuracy = 0.1)))
